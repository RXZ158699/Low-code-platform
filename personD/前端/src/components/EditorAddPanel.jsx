...
function MediaIcon({ type }) {
  if (type === "upload-photo") {
    return (
      <svg viewBox="0 0 48 48" width="36" height="36" aria-hidden="true">
        <rect x="8" y="14" width="32" height="24" rx="4" fill="none" stroke="#6b7280" strokeWidth="2" />
        <path d="M8 32l8-8 6 6 8-10 10 12" fill="none" stroke="#6b7280" strokeWidth="2" />
        <circle cx="17" cy="21" r="2.2" fill="#6b7280" />
        <path d="M24 8v10M20 12h8" stroke="#3b82f6" strokeWidth="2.2" strokeLinecap="round" />
      </svg>
    );
  }
  if (type === "upload-phone") {
    return (
      <svg viewBox="0 0 48 48" width="36" height="36" aria-hidden="true">
        <rect x="16" y="8" width="16" height="28" rx="3" fill="none" stroke="#6b7280" strokeWidth="2" />
        <path d="M24 4v8M20 8h8" stroke="#3b82f6" strokeWidth="2.2" strokeLinecap="round" />
  ...
  return (
    <svg viewBox="0 0 48 48" width="36" height="36" aria-hidden="true">
      <rect x="10" y="14" width="28" height="22" rx="3" fill="none" stroke="#6b7280" strokeWidth="2" />
      <path d="M10 30l8-7 6 5 7-8 7 10" fill="none" stroke="#6b7280" strokeWidth="2" />
      <path d="M34 8l1.2 2.8L38 12l-2.8 1.2L34 16l-1.2-2.8L30 12l2.8-1.2L34 8z" fill="#f59e0b" />
    </svg>
  );
}

function ShapeIcon({ id }) {
  if (id === "square") return <span className="editor-add-shape-mark is-square" />;
  if (id === "triangle") return <span className="editor-add-shape-mark is-triangle" />;
  if (id === "circle") return <span className="editor-add-shape-mark is-circle" />;
  if (id === "pentagon") {
    return (
      <svg viewBox="0 0 24 24" width="18" height="18" aria-hidden="true">
        <polygon points="12,3 21,10 17.5,21 6.5,21 3,10" fill="none" stroke="#4b5563" strokeWidth="1.8" />
      </svg>
    );
  }
...
function ComponentIcon({ id }) {
  ...
  if (id === "magnifier") {
    return (
      <svg viewBox="0 0 32 32" width="28" height="28" aria-hidden="true">
        <circle cx="14" cy="14" r="7" fill="none" stroke="#3b82f6" strokeWidth="2.4" />
        <path d="M19 19l7 7" stroke="#3b82f6" strokeWidth="2.4" strokeLinecap="round" />
      </svg>
    );
  }
  if (id === "table") {
    return (
      <svg viewBox="0 0 32 32" width="28" height="28" aria-hidden="true">
        <rect x="5" y="7" width="22" height="18" rx="2" fill="none" stroke="#64748b" strokeWidth="2" />
  ...
  if (id === "chart") {
    return (
      <svg viewBox="0 0 32 32" width="28" height="28" aria-hidden="true">
        <circle cx="16" cy="16" r="9" fill="none" stroke="#fb923c" strokeWidth="6" strokeDasharray="16 40" />
        <circle cx="16" cy="16" r="9" fill="none" stroke="#60a5fa" strokeWidth="6" strokeDasharray="12 44" strokeDashoffset="-16" />
      </svg>
    );
  }
  return (
...
export default function EditorAddPanel({ open, onClose, onSelect }) {
  return (
    <div
    ...
    >
      <div className="editor-add-panel-body">
        <section className="editor-add-section">
          ...
          <div className="editor-add-media-grid">
            {MEDIA_ITEMS.map((item) => (
              <button type="button" key={item.id} className="editor-add-card" onClick={() => onSelect(item.id)}>
        ...
        <section className="editor-add-section">
          <div className="editor-add-section-head">
            <h3>文字</h3>
            <button type="button" className="editor-add-more" onClick={() => onSelect("text-more")}>
              查看更多
            </button>
          </div>
        ...
        <section className="editor-add-section">
          ...
          <div className="editor-add-draw">
            ...
            <button type="button" className="editor-add-doodle" aria-label="涂鸦笔" onClick={() => onSelect("doodle")}>
              ...
              <svg viewBox="0 0 48 48" width="34" height="34" aria-hidden="true">
                <path
                  ...
                  strokeLinecap="round"
                />
                <path d="M30 14l6 6" stroke="#f59e0b" strokeWidth="2" strokeLinecap="round" />
              </svg>
              <span>涂鸦笔</span>
            </button>
          </div>
      ...
      <button type="button" className="editor-add-collapse" aria-label="收起添加面板" onClick={onClose}>
